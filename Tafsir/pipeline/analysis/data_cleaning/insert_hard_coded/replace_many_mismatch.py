...
# --- DEINE KONSTANTEN & IMPORTS (Hier vereinfacht angenommen) ---
# Stelle sicher, dass ALL_COLUMNS, TAGS etc. definiert sind wie in deinem Originalskript
...
from Tafsir.pipeline.analysis.data_cleaning.replace_hard_coded.missing_rows_from_sqlite3_viewer import (
    extract_tafsir_sections,
    html_input,
)
...
def update_specific_tafsir_entry(
    engine, section_table, block_table, chunk_table, raw_text, target_id
):
    if not raw_text:
        return
...
hard_coded_response = extract_tafsir_sections(html_input)
...
if __name__ == "__main__":
    ...
    print(f"Repariere ID {ID} in {target_db}...")
    for i in hard_coded_response:
