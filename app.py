...
ANNOTATED_DIR = Path(os.getenv("ANNOTATED_DIR", "Tafsir/tafsir_books_annotated"))
SOURCE_DIR = Path(os.getenv("SOURCE_DIR", "Tafsir/tafsir_books"))
...
class TafsirRepository:
    ...
    def _fetch_xml_for_mode(
        ...
        if mode == "blocks":
            ...
            return "\n\n".join(r["block"] for r in rows if r["block"])
        if mode == "chunks":
            rows = self.annotated_conn.execute(
                f"""
                SELECT c.chunk
            ...
            return "\n\n".join(r["chunk"] for r in rows if r["chunk"])
    ...
    def search(
        ...
        for row in rows:
            rec = self._record_from_row(
                row, allowed_tags=allowed_tags, mode=mode, highlight=highlight
            )
            if (
...
def _validate_mode(mode: str) -> str:
    ...
    if mode not in {"full", "blocks", "chunks"}:
        raise HTTPException(
            status_code=400, detail="mode must be one of: full, blocks, chunks"
        )
    return mode
