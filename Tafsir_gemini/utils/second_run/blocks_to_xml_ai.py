...
def _write_log(path_or_entry, entry=None):
    """
    Write a log entry. Accepts either a single string (written to log.txt in CWD)
    or a path plus entry (relative paths are rooted at CWD).
    """
    if entry is None:
        log_file = Path.cwd() / "log.txt"
        message = path_or_entry
    else:
        log_file = Path(path_or_entry)
...
def _get_gemini_client():
    """
    Lazy-load Gemini client to avoid hard dependency during import and reuse across calls.
    """
    global _GEMINI_CLIENT, _GENAI_TYPES
    if _GEMINI_CLIENT is None or _GENAI_TYPES is None:
        try:
            from google import genai
            from google.genai import types
        except ImportError as exc:
            raise RuntimeError(
                "Gemini client not available. Install google-genai or set PYTHONPATH accordingly."
            ) from exc
...
def request_gemini_response(prompt):
    """
    Submit the prompt to the Gemini API and return the raw text response.
    Uses a pre-created cached context whose name is provided via GEMINI_CACHE_NAME.
    """
    client, types = _get_gemini_client()
    if not GEMINI_CACHE_NAME:
        raise RuntimeError("GEMINI_CACHE_NAME is not set; cannot load cached context.")
    if not MODEL_ID:
        raise RuntimeError("GEMINI_MODEL_ID is not set in environment/.env")

    config = types.GenerateContentConfig(cached_content=GEMINI_CACHE_NAME)
...
def cleanup_cycle(
    ...
    """
    extracted_text = prefetched_text if prefetched_text is not None else None
            ...
            for row in results:
                ...
                while attempts <= GUARD_MAX_RETRIES:
                    ...
                    if not extracted_text:
                        print("Keine Antwort von der Gemini API erhalten; erneut versuchen.")
                        _write_log(
                            "logs/progress.log",
