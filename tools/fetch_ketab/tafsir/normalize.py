...
def normalize_arabic(text: str) -> str:
    ...
    # 5) Normalisierung von 'Ibn' -> 'Bin' (Präfix-erhaltend)
    # \1 fängt das optionale و oder ف ein und setzt es wieder davor
    text = regex.sub(r"(?<![\p{Arabic}])([وف])?ابن(?![\p{Arabic}])", r"\1بن", text)

    # 6) Normalisierung von 'Aby' -> 'Abu' (Präfix-erhaltend)
    text = regex.sub(r"(?<![\p{Arabic}])([وف])?ابي(?![\p{Arabic}])", r"\1ابو", text)
