...
HADITH_CONFIG = {
    "isnad_comparison": {  # Überliefererketten (optional, derzeit nicht im Einsatz)
        "anchor_n": 3,
...
if __name__ == "__main__":
    # Beispiel 1: Hadith mit 80 Wörtern
    word_count = 80
    category = get_text_category(word_count)  # → "medium"
    n = calculate_n_value(word_count, category)  # → 80 * 0.2 = 16, aber max ist 15
    print(f"Text mit {word_count} Wörtern → Kategorie: {category}, n-gram Größe: {n}")

    # Beispiel 2: Kurzer Hadith mit 25 Wörtern
    word_count = 25
    category = get_text_category(word_count)  # → "short"
    n = calculate_n_value(word_count, category)  # → 25 * 0.25 = 6.25 → 6
    print(f"Text mit {word_count} Wörtern → Kategorie: {category}, n-gram Größe: {n}")
...
RECOMMENDATIONS = {
    ...
    "hadith_chain_comparison": {
        ...
        "x_values": [0.25, 0.35],  # Höher wegen spezifischer Namen
        "metric": "overlap",
        "threshold": 0.7,
    },
}
